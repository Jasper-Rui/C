...
int main() {
    ...
    int* arrayp = array;
    int i = 0;
    int szz = sizeof(array)/sizeof(array[0]);
    for(i = 0; i < szz; i++){
        *(arrayp + i) = i;
    }
    ...
    printf("\n");
    int block[10] = {0};
    printf("%p \n", block); //array name here is the 1st element address
    //0061FE08
    printf("%p \n", block + 1);

    printf("%p \n", &block[0]); //here is the 1st element address
    //0061FE08
    printf("%p \n", &block[0] + 1);

    printf("%p \n", &block); //here is the address of the whole array address
    //0061FE08
    printf("%p \n", &block + 1);
    printf("\n");

    int size = sizeof(&block);
    //(&block) is the address of block
    //here the sizeof a address is 4
    printf("%d \n", size);

    int sizez = sizeof(block);
    //here is the size of whole array
